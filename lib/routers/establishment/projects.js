...
router.get('/:id/question/:question', (req, res, next) => {
  ...
  Promise.resolve()
    ...
    .then(versions => {
      ...
      // The comparison component should only _ever_ be visible when looking at the most recent version.
      // Which means, I only need the version one before
      ...
      // only get versions created after last granted, if granted
        ...
        .filter(version => version.status === 'submitted' || version.status === 'granted')[1];

      res.response = {
        previous: previous ? getNode(previous.data, req.params.question) : undefined,
        granted: granted ? getNode(granted.data, req.params.question) : undefined
      };
      next();
    })
    .catch(next);
