...
const getPil = (req, res, next) => {
  ...
  if (!req.user.profile.asruUser) {
    ...
    if (pilContainer.establishment && !profileEsts.includes(pilContainer.establishment.id)) {
      delete pilContainer.establishment;
    }
    trainingPils.forEach(p => {
      if (!profileEsts.includes(p.trainingCourse.establishmentId)) {
        delete p.trainingCourse.establishment;
        delete p.trainingCourse.project;
      }
    });
  }
